...
const char* solveLinearSystemKernelSource =
"__kernel void solveLinearSystem(__global float* L, __global float* U, __global float* B, __global float* result, const int size) {\n"
"    int i, j;\n"
"    float sum;\n"
"\n"
...
"        sum = 0.0f;\n"
"        for (j = 0; j < i; ++j) {\n"
"            sum += L[i * size + j] * result[j];\n"
"        }\n"
"        result[i] = (B[i] - sum) / L[i * size + i];\n"
"    }\n"
"\n"
...
"        }\n"
"        result[i] = (result[i] - sum) / U[i * size + i];\n"
"    }\n"
"}\n";
